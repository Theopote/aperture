...
Aperture 的完整产品路线图，将项目拆分为 **12 个可交付阶段**。每个阶段有明确的目标、交付物、依赖关系与验收标准。
...
- Undo / Redo（`EditCommand` 模式）
- History 时间线（per-instance revision log）
- Generate（提交）按钮与校验反馈
- 未来 AI 命令栏占位
...
| 模块 | 包 |
|---|---|
| `aperture-core` | `editor/DesignSession`, `EditCommand`, `EditHistory`, `SnapEngine` |
