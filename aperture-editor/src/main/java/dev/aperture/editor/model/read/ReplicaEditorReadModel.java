...
public final class ReplicaEditorReadModel implements EditorReadModel {
	...
	private final Function<ReplicaObject,List<RuntimeActionDescriptor>> runtimeActions;
	public ReplicaEditorReadModel(ClientReplicaStore replicas, PreviewCoordinator previews, DiagnosticsModel diagnostics) {
		this(replicas,previews,diagnostics,ignored -> List.of());
	}
	public ReplicaEditorReadModel(ClientReplicaStore replicas, PreviewCoordinator previews, DiagnosticsModel diagnostics, Function<ReplicaObject,List<RuntimeActionDescriptor>> runtimeActions) {
		this.replicas=Objects.requireNonNull(replicas); this.previews=Objects.requireNonNull(previews); this.diagnostics=Objects.requireNonNull(diagnostics); this.runtimeActions=Objects.requireNonNull(runtimeActions);
	}
	public Optional<ObjectEditorView> object(ArchitecturalObjectId id) { return replicas.replica(id).map(replica -> {
		...
		return new ObjectEditorView(id,instance.typeId(),instance.familyId(),instance.metadata().getOrDefault("displayName",instance.typeId().toString()),
			instance.transform(),instance.hostBindings(),ParameterSet.of(values),replica.state(),instance.revision(),replica.state().revision().value(),status,runtimeActions.apply(replica),diagnostics.forObject(id));
	}); }
