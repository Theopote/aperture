...
public final class DefaultParameterEditSession implements ParameterEditSession {
	...
	public EditorCommandSubmission commit(){ensureActive();active=false;var result=gateway.submitParameterEdit(id,key,authoritative,preview,revision);if(result.status()==EditorCommandSubmission.Status.PENDING)previews.associate(result.commandId(),id,key);else previews.clear(id,key);return result;}
