	...
	public static OpeningTypeDefinition casementWindow() {
		return OpeningTypeDefinition.builder(FIXED_WINDOW_ID, OpeningCategory.WINDOW, RECTANGULAR_WINDOW_GENERATOR)
			...
			.parameter("frame_width", ParameterDefinition.builder(ParameterType.LENGTH)
				...
				.max(150)
				.build())
			.parameter("frame_depth", ParameterDefinition.builder(ParameterType.LENGTH)
				.defaultValue(ParameterValue.length(80))
				.min(30)
				.max(200)
				.build())
			.parameter("open_angle", ParameterDefinition.builder(ParameterType.ANGLE)
				.defaultValue(ParameterValue.angle(0))
				.min(0)
				.max(90)
				.build())
			.materialSlot("frame")
